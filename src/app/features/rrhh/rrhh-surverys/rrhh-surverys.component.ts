...
import { EnabledEvaluationService } from '../../../services/nestjs-services/enabledEvaluationService';
...
export class RrhhSurverysComponent implements OnInit {
  ...
  createEnabledEvaluation() {
    if (this.month && this.year) {
      this.enabledEvaluationService
        .createEnableEvaluation({ month: this.month.value, year: this.year })
        .subscribe({
          next: (response) => {
          ...
          error: (error) => {
            console.error(error);
            alert(
              'Error al crear evaluación (Probablemente ya existe una evaluación con las fechas dadas)'
            );
          },
        });
    }
