...
export class AdminConfirmationDeleteUserComponent implements OnInit{
  ...
  userId!:number
  selectedUser!:any;
  constructor(private ref:DynamicDialogRef,private userService:UserService,private oppService:OpportunityService,private config:DynamicDialogConfig){}
  ngOnInit(): void {
  ...
  changeAllOppToNewUser(newUserId:number){
    this.oppService.changeAllOppToNewUser({userId:this.userId,newUserId}).pipe(
      
      switchMap(()=>this.userService.deleteUser(this.userId))
    ).subscribe({
      next:response=>{
